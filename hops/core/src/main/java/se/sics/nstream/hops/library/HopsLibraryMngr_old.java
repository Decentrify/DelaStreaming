...
// */
//public class HopsLibraryMngr_old {
//
//  private static final Logger LOG = LoggerFactory.getLogger(
//    HopsLibraryMngr.class);
...
//
//  public HopsLibraryMngr_old(ComponentProxy proxy, Config config, String logPrefix,
//    KAddress selfAdr) {
...
//
//    libOpTracker = new LibOpTracker(logPrefix, config, componentHelper,
//      endpointTracker, transferTracker, library, selfAdr.getId());
//  }
//
//  public void start() {
//    libOpTracker.restartTorrents();
//  }
//
//  public void close() {
//  }
...
//  public static interface EndpointCallback extends
//    BasicCompleteCallback<Boolean> {
//
//    public void setEndpoints(Map<String, Identifier> nameToId,
//      Map<Identifier, StreamEndpoint> endpoints);
//  }
//
...
//    //**********************************************************************
//    private final Map<Identifier, List<EndpointCallbackWrapper>> pendingEndpoints
//      = new HashMap<>();
//
...
//
//      helper.proxy.
//        subscribe(handleEndpointConnected, helper.endpointControlPort);
//    }
//
//    private void registerEndpoints(List<DurableStorageProvider> storageEndpoints,
//      final EndpointCallback callback) {
//      EndpointCallbackWrapper callbackWrapper = new EndpointCallbackWrapper(
//        callback);
...
//          callbackWrapper.addConnected(endpointName, endpointId, dsp.
//            getEndpoint());
//        } else {
//          LOG.info("{}endpoint:{} waiting", logPrefix, endpointName);
//          Identifier endpointId = endpointIdRegistry.register(endpointName);
//          callbackWrapper.
//            addWaiting(endpointName, endpointId, dsp.getEndpoint());
//          List<EndpointCallbackWrapper> waiting = pendingEndpoints.get(
//            endpointId);
//          if (waiting == null) {
...
//          helper.proxy.trigger(new DEndpointConnect.Request(endpointId, dsp),
//            helper.endpointControlPort);
//        }
//      }
...
//        List<EndpointCallbackWrapper> waiting = pendingEndpoints.remove(
//          resp.req.endpointId);
//        if (waiting != null) {
...
//    }
//
//    public void addWaiting(String endpointName, Identifier endpointId,
...
//    }
//
//    public void addConnected(String endpointName, Identifier endpointId,
//      StreamEndpoint endpoint) {
//      nameToId.put(endpointName, endpointId);
//      endpoints.put(endpointId, endpoint);
//    }
...
//
//  //*****************************TRANSFER_START*******************************
//  public static interface TransferCallbackWrapper<V extends Object> extends
//    BasicCompleteCallback<V> {
//
//    /**
//     * @param result
...
//     * @return false if should stop
//     */
//    public boolean manifest(Result<MyTorrent.Manifest> transferResult,
//      AdvanceTransferCallback callback);
//  }
//
...
//    private final ComponentHelper helper;
//    //**********************************************************************
//    private final Map<OverlayId, TransferCallbackWrapper<Boolean>> pendingTransferStarts
//      = new HashMap<>();
//
...
//    }
//
//    public void startTransfer(OverlayId torrentId,
//      TransferCallbackWrapper<Boolean> callback) {
//      LOG.info("{}torrent:{} - starting", logPrefix, torrentId);
//      StartTorrent.Request req = new StartTorrent.Request(torrentId, callback.
//        getPeers());
...
//        }
//
//        TransferCallbackWrapper callback = pendingTransferStarts.get(
//          resp.torrentId);
...
//        } else {
//          LOG.debug("{}torrent:{} - getting raw torrent", logPrefix,
//            resp.torrentId);
...
//        if (resp.result.isSuccess()) {
//          LOG.debug("{}torrent:{} - get raw torrent - received, saving...",
//            logPrefix, resp.overlayId());
//        } else {
//          LOG.debug("{}torrent:{} - get raw torrent - failed", logPrefix, resp.
//            overlayId());
//        }
//        TransferCallbackWrapper mainCallback = pendingTransferStarts.get(resp.
//          overlayId());
//        AdvanceTransferCallback advanceCallback
//          = new AdvanceTransferCallback() {
//            @Override
...
//        TransferCallbackWrapper callback = pendingTransferStarts.get(resp.
//          overlayId());
//        if (resp.result.isSuccess()) {
//          LOG.debug("{}torrent:{} - transfer - set up", logPrefix, resp.
//            overlayId());
//        } else {
//          LOG.debug("{}torrent:{} - transfer - set up failed", logPrefix, resp.
//            overlayId());
//        }
...
//
//    public BasicTransfer(MyTorrent torrent, List<KAddress> peers,
//      BasicCompleteCallback<Boolean> complete) {
...
//
//    @Override
//    public boolean manifest(Result<MyTorrent.Manifest> result,
//      AdvanceTransferCallback callback) {
...
//  }
//
//  public static class ExtendedTransfer implements
//    TransferCallbackWrapper<Boolean> {
//
...
//
//    public ExtendedTransfer(MyExtendedHelper helper,
//      MyTorrentBuilder torrentBuilder,
//      List<KAddress> peers, MyStream manifestStream,
//      BasicCompleteCallback<Boolean> complete) {
//      this.helper = helper;
...
//    @Override
//    public boolean manifest(Result<MyTorrent.Manifest> transferResult,
//      final AdvanceTransferCallback callback) {
//      if (!transferResult.isSuccess()) {
//        throw new RuntimeException("todo cleanup");
//      }
//      Result<Boolean> writeResult = helper.manifestHelper().writeManifest(
//        manifestStream.endpoint, manifestStream.resource, transferResult.
//        getValue());
//      if (!writeResult.isSuccess()) {
//        throw new RuntimeException("todo cleanup");
//      }
//      Either<MyTorrent.Manifest, ManifestJSON> manifestResult = Either.left(
...
//          @Override
//          public void ready(
//            Result<Pair<KafkaEndpoint, Map<String, KafkaResource>>> kafkaDetails) {
//              if (kafkaDetails.isSuccess()) {
//                Map<FileId, FileExtendedDetails> extendedDetails
//                = helper.manifestHelper().extendedDetails(
//                  torrentBuilder, kafkaDetails.getValue().
//                  getValue0(), kafkaDetails.getValue().
//                  getValue1());
...
//
//    public Result<ManifestJSON> readManifest(StreamEndpoint endpoint,
//      StreamResource resource);
//
//    public Result<Boolean> writeManifest(StreamEndpoint endpoint,
//      StreamResource resource, MyTorrent.Manifest result);
//
//    public Map<FileId, FileExtendedDetails> extendedDetails(
//      MyTorrentBuilder torrentBuilder, KafkaEndpoint kafkaEndpoint,
//      Map<String, KafkaResource> kafkaDetails);
//  }
//
//  public static interface MyExtendedDetailsHelper {
//
//    public void extendedDetails(
//      BasicCompleteCallback<Pair<KafkaEndpoint, Map<String, KafkaResource>>> complete);
//  }
//
...
//
//    public void setEndpoints(Map<String, Identifier> endpointNameToId,
//      Map<Identifier, StreamEndpoint> endpoints);
//
...
//
//    public void setExtendedDetails(
//      Map<FileId, FileExtendedDetails> extendedDetails);
//
//    public MyTorrent getTorrent();
//  }
...
//      }
//      Pair< Map< String, FileId>, Map<FileId, FileBaseDetails>> aux
//        = ManifestHelper.getBaseDetails(torrentId, manifestJSON,
...
//    private final MyTorrentBuilder torrentBuilder;
//
//    public BasicTorrent(EndpointTracker endpointTracker,
//      TransferTracker transferTracker, MyBasicHelper helper,
//      BasicCompleteCallback<Boolean> completeCallback,
//      OverlayId torrentId, MyStream manifestStream,
//      List<KAddress> torrentPeers, MyTorrentBuilder torrentBuilder) {
...
//      DurableStorageProvider storageProvider = helper.storageHelper().
//        getStorageProvider(manifestStream.endpoint);
...
//          Identifier manifestEndpointId = nameToId.get(manifestStream.endpoint.
//            getEndpointName());
//          StreamId manifestStreamId = TorrentIds.streamId(manifestEndpointId,
//            TorrentIds.fileId(torrentId, MyTorrent.MANIFEST_ID));
...
//      final Result<ManifestJSON> manifestJSON = helper.manifestHelper().
//        readManifest(manifestStream.endpoint, manifestStream.resource);
//      if (!manifestJSON.isSuccess()) {
//        throw new RuntimeException("cleanup - todo");
//      }
//      Either<MyTorrent.Manifest, ManifestJSON> manifestResult = Either.right(
//        manifestJSON.getValue());
//      torrentBuilder.setManifest(manifestResult);
//      Map<FileId, FileExtendedDetails> extendedDetails
//        = helper.manifestHelper().extendedDetails(torrentBuilder, null,
//          new HashMap<String, KafkaResource>());
//      torrentBuilder.setExtendedDetails(extendedDetails);
//      BasicCompleteCallback<Boolean> setupComplete
//        = new BasicCompleteCallback<Boolean>() {
//          @Override
//          public void ready(Result<Boolean> result) {
...
//      TransferCallbackWrapper setupTransfer = new BasicTransfer(torrent,
//        torrentPeers, setupComplete);
...
//
//    public ExtendedTorrent(EndpointTracker endpointTracker,
//      TransferTracker transferTracker, MyExtendedHelper helper,
//      BasicCompleteCallback<Boolean> completeCallback,
//      OverlayId torrentId, MyStream manifestStream,
//      List<KAddress> torrentPeers, MyTorrentBuilder torrentBuilder) {
//      this.endpointTracker = endpointTracker;
...
//          }
//        };
//      ExtendedTransfer setupTransfer = new ExtendedTransfer(helper,
//        torrentBuilder, torrentPeers, manifestStream, setupComplete);
//      transferTracker.startTransfer(torrentId, setupTransfer);
//    }
//
//    private void ready(Result<Boolean> result) {
...
//        @Override
//        public Result<Boolean> writeManifest(StreamEndpoint endpoint,
//          StreamResource resource, MyTorrent.Manifest manifest) {
//          ManifestJSON manifestJSON = ManifestHelper.getManifestJSON(
//            manifest.manifestByte);
//          Result<Boolean> result = DiskHelper.writeManifest(
//            (DiskResource) resource, manifestJSON);
//          return result;
//        }
//
//        @Override
...
//          Pair<StreamId, MyStream> manifestStream = torrentBuilder.
//            getManifestStream();
//          DiskResource manifestResource = (DiskResource) manifestStream.
//            getValue1().resource;
//          for (Map.Entry<String, FileId> file : torrentBuilder.getFiles().
//            entrySet()) {
//            StreamId fileStreamId = manifestStream.getValue0().withFile(file.
//              getValue());
//            DiskResource fileResource = manifestResource.withFile(file.getKey());
//            MyStream fileStream = manifestStream.getValue1().withResource(
//              fileResource);
//            extendedDetails.put(file.getValue(), new DiskFED(fileStreamId,
//              fileStream));
//          }
...
//            hdfsEndpoint.user);
//          return HDFSHelper.readManifest(ugi, hdfsEndpoint,
...
//          UserGroupInformation ugi = UserGroupInformation.createRemoteUser(
//            hdfsEndpoint.user);
//          Result<Boolean> result = HDFSHelper.writeManifest(ugi, hdfsEndpoint,
//            (HDFSResource) resource, manifestJSON);
//          return result;
//        }
//
//        @Override
...
//          Pair<StreamId, MyStream> manifestStream = torrentBuilder.
//            getManifestStream();
//          HDFSResource manifestResource = (HDFSResource) manifestStream.
//            getValue1().resource;
//          for (Map.Entry<String, FileId> file : torrentBuilder.getFiles().
//            entrySet()) {
//            StreamId fileStreamId = manifestStream.getValue0().withFile(file.
...
//            extendedDetails.put(file.getValue(), new HopsFED(Pair.with(
//              fileStreamId, fileStream), kafkaStream));
//          }
//          return extendedDetails;
//        }
//      };
...
//
//    private static final HopsTorrentUploadEvent.Request RESTART_NOHUP_REQ
//      = new HopsTorrentUploadEvent.Request(null, null, null, null, null);
//
//    private final String logPrefix;
...
//
//    public LibOpTracker(String logPrefix, Config config, ComponentHelper comp, EndpointTracker endpointTracker,
//      TransferTracker transferTracker, Library library, Identifier selfId) {
//      this.logPrefix = logPrefix;
//      this.comp = comp;
//      this.endpointTracker = endpointTracker;
//      this.transferTracker = transferTracker;
...
//        if (torrent.getValue().getManifestStream().endpoint instanceof DiskEndpoint) {
//          LOG.info("{}restarting DISK torrent:{} id:{}", logPrefix, torrent.getValue().torrentName, torrent.getKey());
//          restartDiskUpload(torrent.getValue().projectId, torrent.getKey(),
//            torrent.getValue().torrentName, torrent.getValue().getManifestStream());
//        } else {
//          LOG.info("{}restarting HDFS torrent:{}", logPrefix, torrent.getValue().torrentName);
//          restartHDFSUpload(torrent.getValue().projectId, torrent.getKey(), torrent.getValue().torrentName, torrent.
//            getValue().getManifestStream());
//        }
//      }
//    }
//
//    private boolean restartable(TorrentState torrentState) {
//      return torrentState.equals(TorrentState.UPLOADING);
//    }
//
...
//            break;
//          default:
//            comp.proxy.answer(req, req.succes(new TorrentExtendedStatus(req.torrentId, ts, 0, 0)));
//        }
//      }
//    };
//
...
//        TorrentExtendedStatusEvent.Request req = pendingTESE.remove(
//          resp.req.torrentId);
//        if (req == null) {
//          return;
//        }
...
//          case DOWNLOADING:
//          case UPLOADING:
//            comp.proxy.answer(req, req.succes(new TorrentExtendedStatus(req.torrentId, resp.result.torrentStatus,
...
//    }
//
//    private void uploadTorrent(final ResourceSetup setup, final OverlayId torrentId, final String projectId,
//      final String torrentName, final MyStream torrentStream, MyBasicHelper torrentHelper, List<KAddress> torrentPeers,
//      MyTorrentBuilder torrentBuilder) {
//
//      Result r1 = TorrentChecker.prepareUpload(library, projectId, torrentId, torrentName);
//      if (!r1.isSuccess()) {
//        setup.fail(r1);
//        return;
//      }
//      library.prepareUpload(projectId, torrentId, torrentName);
//      setup.addCleanup(new LibraryCleanup(library, torrentId));
//
//      //TODO Alex fix as one flow
//      BasicCompleteCallback<Boolean> completeCallback = new BasicCompleteCallback<Boolean>() {
//        @Override
//        public void ready(Result<Boolean> r2) {
//          pendingDiskUploads.remove(torrentId);
//          if (!r2.isSuccess()) {
//            setup.fail(r2);
//          }
//          Result r3 = TorrentChecker.upload(library, projectId, torrentId, torrentName);
//          if (!r3.isSuccess()) {
//            setup.fail(r3);
//          }
//
//          library.upload(projectId, torrentId, torrentName, torrentStream);
//          setup.success(Result.success(true));
//        }
//      };
//
//      BasicTorrent upload = new BasicTorrent(endpointTracker, transferTracker, torrentHelper, completeCallback,
//        torrentId, torrentStream, torrentPeers, torrentBuilder);
//      pendingDiskUploads.put(torrentId, req);
//      upload.start();
//
//    }
//
...
//        final MyStream torrentStream = new MyStream(new DiskEndpoint(), new DiskResource(req.manifestResource.dirPath,
...
//              if (!uResult.getValue()) {
//                LOG.warn("{}should not happen - might have double started something - investigate", logPrefix);
//              }
//              comp.proxy.answer(req, req.success(result));
//            } else {
//              //fail on library - mismatches - clean sink/sources - library cleaned only on explicit
//              cleanSinksAndSources(req.torrentId);
//              comp.proxy.answer(req, req.failed(result));
//            }
//          }
...
//    };
//
//    private static interface CleanupResource {
//
//      public void doCleanup();
//    }
//
//    private static class LibraryCleanup implements CleanupResource {
//
//      private final Library lib;
//      private final OverlayId torrentId;
//
//      public LibraryCleanup(Library lib, OverlayId torrentId) {
//        this.lib = lib;
//        this.torrentId = torrentId;
//      }
//
...
//    }
//
//    private static class StorageCleanup implements CleanupResource {
//
//      @Override
//      public void doCleanup() {
...
//    }
//
//    private static interface Action<C extends Object> {
//
//      public void perform(Result<C> result);
//    }
//
//    private static abstract class ResourceMngr {
//
//      private final List<CleanupResource> cleanupResources = new LinkedList<>();
//
//      public void addCleanup(CleanupResource cleanupResource) {
//        cleanupResources.add(cleanupResource);
//      }
//
//      protected void cleanup() {
//        for (CleanupResource cr : cleanupResources) {
//          cr.doCleanup();
//        }
//      }
//    }
//
//    private static abstract class ResourceSetup extends ResourceMngr {
//
//      private final Action onSucces;
//      private final Action onFail;
//
//      public ResourceSetup(Action onSucces, Action onFail) {
...
//    }
//
//    public void restartDiskUpload(final String projectId, final OverlayId torrentId, final String torrentName,
//      final MyStream manifestStream) {
//
//      Result puResult = library.prepareUpload(projectId, torrentId, torrentName);
//      if (!puResult.isSuccess()) {
...
//            return;
//          }
//          LOG.info("{}DISK torrent restart - success name:{} id:{}", new Object[]{logPrefix, torrentName, torrentId});
//        }
//      };
//
//      BasicTorrent upload = new BasicTorrent(endpointTracker, transferTracker, torrentHelper, completeCallback,
//        torrentId, manifestStream, torrentPeers, torrentBuilder);
...
//    };
//
//    public Result restartHDFSUpload(final String projectId,
//      final OverlayId torrentId, String torrentName,
//      final MyStream manifestStream) {
...
//        };
//
//      BasicTorrent upload = new BasicTorrent(endpointTracker, transferTracker,
//        torrentHelper, completeCallback, torrentId, manifestStream,
//        torrentPeers, torrentBuilder);
//      pendingHDFSUploads.put(torrentId, RESTART_NOHUP_REQ);
//      upload.start();
//    }
//
//    Handler handleDiskTorrentDownload
//      = new Handler<HopsTorrentDownloadEvent.StartRequest>() {
//        @Override
...
//          OverlayId torrentId = req.torrentId;
//          final MyStream torrentStream = new MyStream(new DiskEndpoint(),
//            new DiskResource(req.manifest.dirPath,
//              req.manifest.fileName));
//          List<KAddress> torrentPeers = req.partners;
//          MyTorrentBuilder torrentBuilder = new MyTorrentBuilderImpl(
//            torrentId);
//
...
//                  complete.ready(Result.success(Pair.with(null,
//                        new HashMap<String, KafkaResource>())));
//                }
//              };
//            }
...
//          };
//
//          ExtendedTorrent upload = new ExtendedTorrent(endpointTracker,
//            transferTracker, torrentHelper, completeCallback,
//            torrentId, torrentStream, torrentPeers, torrentBuilder);
...
//      };
//
//    Handler handleHDFSTorrentDownload
//      = new Handler<HopsTorrentDownloadEvent.StartRequest>() {
//        @Override
//        public void handle(final HopsTorrentDownloadEvent.StartRequest req) {
//          LOG.trace("{}received:{}", logPrefix, req);
...
//          library.prepareDownload(req.torrentId, req.torrentName);
//          OverlayId torrentId = req.torrentId;
//          final MyStream torrentStream = new MyStream(req.hdfsEndpoint,
//            req.manifest);
//          List<KAddress> torrentPeers = req.partners;
//          MyTorrentBuilder torrentBuilder = new MyTorrentBuilderImpl(
//            torrentId);
...
//            public void ready(Result<Boolean> result) {
//              if (result.isSuccess()) {
//                HopsTorrentDownloadEvent.AdvanceRequest areq
...
//      };
//
//    Handler handleHDFSTorrentAdvanceDownload
//      = new Handler<HopsTorrentDownloadEvent.AdvanceRequest>() {
//        @Override
//        public void handle(HopsTorrentDownloadEvent.AdvanceRequest req) {
//          LOG.trace("{}received:{}", logPrefix, req);
//
//          pendingHDFSAdvanceDownloads.put(req.torrentId, req);
//          BasicCompleteCallback<Pair<KafkaEndpoint, Map<String, KafkaResource>>> advanceCallback
//          = pendingHDFSDownloadCallback.remove(req.torrentId);
//          KafkaEndpoint kafkaEndpoint = req.kafkaEndpoint.isPresent()
//            ? req.kafkaEndpoint.get() : null;
//          Map<String, KafkaResource> kafkaResources = kafkaEndpoint
//          != null ? req.kafkaDetails
//            : new HashMap<String, KafkaResource>();
//          advanceCallback.ready(Result.success(Pair.with(kafkaEndpoint,
//                kafkaResources)));
//        }
//      };
//
//    private Result<Boolean> validateDownloadRequest(
...
//          } else {
//            return Result.
//              badArgument("existing - download details do not match");
//          }
//        case PREPARE_UPLOAD:
//        case UPLOADING:
//          return Result.
//            badArgument("expected NONE/DOWNLOADING, found:" + status);
//        case DESTROYED:
//        case NONE:
...
//        Result fResult = library.finishDownload(ind.torrentId);
//        if (!fResult.isSuccess()) {
//          LOG.warn("{}investigate - failed finished result", logPrefix);
//        }
//      }
//    };
//  }
