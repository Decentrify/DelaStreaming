...
import se.sics.dela.workers.ctrl.util.NxDataStreamIdExtractor;
...
//  private Component netConnMngr;
  ...
  Handler handleStart = new Handler<Start>() {
    ...
    public void handle(Start event) {
      ...
      nxNetProxy = new NxNetProxy()
        .setup(proxy, logger, networkConfig, eventIds, privateIpDetected())
        .ledbatSetup(timerComp.getPositive(Timer.class));
  ...
  private Consumer<InetAddress> privateIpDetected() {
    return (privateIp) -> {
      ...
      nxNetProxy.bindLedbat(selfAdr, networkReady());
    };
  }
  ...
  private Consumer<Boolean> networkReady() {
    return (_ignore) -> {
      ...
      nxNetProxy.connectNetwork(ctrlCenter);
...
//    positiveNetworkMsgs.put(LedbatMsg.MSG_TYPE, new NxDataStreamIdExtractor.Source(netConnStackId));
...
//    nxNetProxy.connectNetwork(netConnMngr);
...
//  }
  ...
  private void createTaskMngrReceiver() {
    ...
    Map<String, MsgIdExtractorV2> positiveNetworkMsgs = new HashMap<>();
    positiveNetworkMsgs.put(LedbatMsg.MSG_TYPE, new NxDataStreamIdExtractor());
    NxStackDefinitionV2.NetworkPort posNetPort = new NxStackDefinitionV2.NetworkPort(positiveNetworkMsgs);

    positivePorts.add((Class) Timer.class);
    Map<String, EventIdExtractorV2> timerEvents = new HashMap<>();
    timerEvents.put(TimerProxyImpl.Timeout.EVENT_TYPE, new NxStackEventIdExtractors.TimerProxy());
    ...
    receiverTasks = create(NxMngrCompV2.class, new NxMngrCompV2.Init("task mngr receiver", stackDefintion, 
      negativePorts, negativeIdExtractors, positivePorts, positiveIdExtractors, Optional.of(posNetPort)));
    connect(receiverTasks.getNegative(Timer.class), timerComp.getPositive(Timer.class), Channel.TWO_WAY);
    nxNetProxy.connectNetwork(receiverTasks);
    nxStackChannel.addChannel(LedbatMsg.StackType.RECEIVER.getId(nxStackIds), receiverTasks.getPositive(NxMngrPort.class));
...
//      Channel.TWO_WAY);
  ...
  private void createTaskMngrSender() {
    ...
    positivePorts.add((Class) LedbatStatus.Port.class);
    Map<String, EventIdExtractorV2> ledbatStatusEvents = new HashMap<>();
    ledbatStatusEvents.put(LedbatStatus.Event.EVENT_TYPE, new NxStackEventIdExtractors.LedbatStatusE());
    positiveIdExtractors.add(ledbatStatusEvents);
    ...
    nxNetProxy.connectNetwork(senderTasks);
    nxNetProxy.connectLedbat(senderTasks);
    nxStackChannel.addChannel(LedbatMsg.StackType.SENDER.getId(nxStackIds), senderTasks.getPositive(NxMngrPort.class));
