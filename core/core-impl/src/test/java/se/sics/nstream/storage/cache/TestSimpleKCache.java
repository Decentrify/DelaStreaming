...
public class TestSimpleKCache {
  ...
  private static void experimentSetup() {
    ...
    req0 = new DStorageRead.Request(eventIds.randomId(), readStream.getValue0(), b0);
    resp0 = req0.respond(Result.success(r));
    req1 = new DStorageRead.Request(eventIds.randomId(), readStream.getValue0(), b1);
    resp1 = req1.respond(Result.success(r));
    req2 = new DStorageRead.Request(eventIds.randomId(), readStream.getValue0(), b2);
  ...
  private SimpleKCache buildCache(Config config, MockComponentProxy proxy, MockExceptionHandler syncExHandler) {
    ...
    SimpleKCache skCache = new SimpleKCache(config, proxy, syncExHandler, 
      new ComponentLoadTracking("test", proxy, new QueueLoadConfig(config)), readStream, eventIds);
