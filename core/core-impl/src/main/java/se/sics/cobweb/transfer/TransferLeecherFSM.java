 ...
 */
public class TransferLeecherFSM {
  private static final Logger LOG = LoggerFactory.getLogger(TransferLeecherFSM.class);
  public static final String NAME = "torrent-leecher-transfer-fsm";
  ...
  public static MultiFSM multifsm(TransferLeecherExternal es, TransferLeecherInternal.Builder isb, OnFSMExceptionAction oexa) throws FSMException {
    Pair<FSMBuilder.Machine, FSMBuilder.Handlers> fp = prepare();

    FSMIdExtractor<TransferLeecherFSMEvent> fsmIdExtractor = new FSMIdExtractor<TransferLeecherFSMEvent>() {
      ...
      @Override
      public Optional<FSMId> fromEvent(FSMDefId fsmdId, TransferLeecherFSMEvent event) throws FSMException {
        if (events.contains(event.getClass()) || positiveNetworkMsgs.contains(event.getClass())
          || negativeNetworkMsgs.contains(event.getClass())) {
          return Optional.of(fsmdId.getFSMId(event.tlFSMId()));
        }
  ...
  private static Pair<FSMBuilder.Machine, FSMBuilder.Handlers> prepare() throws FSMException {
    ...
    FSMBuilder.Machine machine = FSMBuilder.machine()
      .onState(FSMBasicStateNames.START)
        .nextStates(TransferLeecherStates.GETTING_METADATA)
        .buildTransition()
      .onState(TransferLeecherStates.GETTING_METADATA)
        .toFinal()
        .buildTransition();

    FSMBuilder.Handlers handlers = FSMBuilder.events()
      .defaultFallback(TransferLeecherHandlers.handleEventFallback, TransferLeecherHandlers.handleMsgFallback)
      .negativePort(LoopbackPort.class)
        .onEvent(TransferLocalE.LeecherStart.class)
          .subscribe(TransferLeecherHandlers.handleMetadataGet, FSMBasicStateNames.START)
        .buildEvents()
      ;
