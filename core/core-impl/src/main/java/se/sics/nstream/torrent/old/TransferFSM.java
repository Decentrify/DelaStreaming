...
//import se.sics.nstream.transfer.old.MultiFileTransfer;
...
//            
//            updateCacheHint(msg.getHeader().getSource().getId(), req.content.cacheHints);
//            TransferMngr.Reader reader = transferMngr.readFrom(req.content.fileName);
//            final PendingHashReq phr = new PendingHashReq(msg);
//            pendingOutHashes.put(req.content.eventId, phr);
//            for (final Integer hash : req.content.hashes) {
...
//                            if (phr.canAnswer()) {
//                                pendingOutHashes.remove(req.content.eventId);
//                                answer(phr.answer());
//                            }
//                            return true;
//                        }
...
//            cs.router.useUploadSlot(target);
//            
//            LOG.trace("{}received req for b:{},pb:{}", new Object[]{cs.logPrefix, req.content.pieceNr.getValue0(), req.content.pieceNr.getValue1()});
//            updateCacheHint(msg.getHeader().getSource().getId(), req.content.cacheHints);
//            TransferMngr.Reader reader = transferMngr.readFrom(req.content.fileName);
//            if (reader.hasBlock(req.content.pieceNr.getValue0())) {
//                pendingOutPieces.put(req.content.eventId, req.content);
...
//                    @Override
//                    public boolean success(Result<KReference<byte[]>> result) {
//                        pendingOutPieces.remove(req.content.eventId);
//                        answer(msg, req.answer(req.content.success(result.getValue())));
//                        return true;
//                    }
//                };
//                reader.readPiece(req.content.pieceNr, pieceRC);
//            } else {
//                answer(msg, req.content.missingBlock());
//            }
//        }
//
...
//                return req.getContent().content.hashes.size() == hashes.size() + missingHashes.size();
//            }
//
//            public Pair answer() {
//                LedbatMsg.Response<HashGet.Response> resp = req.getContent().answer(req.getContent().content.success(hashes, missingHashes));
...
//        @Override
//        public void handleHashResp(KContentMsg<KAddress, KHeader<KAddress>, LedbatMsg.Response<HashGet.Response>> msg, LedbatMsg.Response<HashGet.Response> resp) {
//            UUID tId = pendingInHashes.remove(resp.content.eventId);
//            if (tId == null) {
...
//            TransferMngr.Writer writer = transferMngr.writeTo(resp.content.fileName);
//            if (resp.content.status.isSuccess()) {
//                for (Map.Entry<Integer, ByteBuffer> hash : resp.content.hashes.entrySet()) {
//                    LOG.debug("{}received hash:{}", cs.logPrefix, hash.getKey());
//                    writer.writeHash(hash.getKey(), hash.getValue().array(), new NopHashWC());
//                }
//                writer.resetHashes(resp.content.missingHashes);
//            } else {
//                writer.resetHashes(resp.content.missingHashes);
//            }
//            tryDownload();
...
//            UUID tId = pendingInPieces.remove(resp.content.eventId);
//            if (tId == null) {
//                LOG.trace("{}late response:{}", cs.logPrefix, resp);
//                return;
//            }
...
//            TransferMngr.Writer writer = transferMngr.writeTo(resp.content.fileName);
//
//            LOG.trace("{}received resp for b:{},pb:{}", new Object[]{cs.logPrefix, resp.content.pieceNr.getValue0(), resp.content.pieceNr.getValue1()});
//            writer.writePiece(resp.content.pieceNr, resp.content.piece.array(), new NopPieceWC());
//            tryDownload();
//        }
//
//        @Override
...
//                ConnMngr router, ComponentLoadTracking componentLoad) {
//            this.proxy = proxy;
//            this.config = config;
