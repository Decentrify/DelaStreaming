...
public class DwnlConnWorkCtrl {
    ...
    private final Map<Integer, BlockDetails> blockDetails = new HashMap<>();
    //**************************************************************************
    ...
    public DwnlConnWorkCtrl() {
    }

    public void add(Map<Integer, BlockDetails> newBlocks) {
        cacheHintChanged = true;
        nextBlocks.addAll(newBlocks.keySet());
        blockDetails.putAll(newBlocks);
    }
    ...
    private void newPendingBlock(int blockNr) {
        BlockDetails bd = blockDetails.remove(blockNr);
