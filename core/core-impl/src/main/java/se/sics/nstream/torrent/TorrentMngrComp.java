...
public class TorrentMngrComp extends ComponentDefinition {
  ...
  Handler handleTorrentStop = new Handler<StopTorrent.Request>() {
    @Override
    public void handle(StopTorrent.Request req) {
      LOG.info("{}stopping torrent:{}", logPrefix, req.torrentId);
      Component torrentComp = torrentComps.remove(req.torrentId);
      compIdToTorrentId.remove(torrentComp.id());
     
      disconnect(torrentComp.getNegative(Timer.class), timerPort);
      networkChannel.removeChannel(req.torrentId, torrentComp.getNegative(Network.class));
