...
public class TransferComp extends ComponentDefinition {
  ...
  private void buildChannels() {
    ...
    connNetworkChannel = One2NChannel.getChannel("torrentConnNetwork", networkPort, overlayIdExtractor,
      TransferCompFilters.connInclusionFilter);
    ChannelIdExtractor<?, Identifier> connIdExtractor = new MsgTorrentConnIdExtractor(new SourceHostIdExtractor());
    transferNetworkChannel = One2NChannel.getChannel("torrentPeerFileNetwork", networkPort, connIdExtractor,
      TransferCompFilters.transferInclusionFilter);
    dwnlConnChannel = One2NChannel.getChannel("torrentDwnlConn", (Negative) dwnlConnPort.getPair(),
      new EventTorrentConnIdExtractor());
    upldConnChannel = One2NChannel.getChannel("torrentUpldConn", (Negative) upldConnPort.getPair(),
      new EventTorrentConnIdExtractor());
  }
  ...
  Handler handleSeederConnectSuccess = new Handler<Seeder.Success>() {
    ...
    public void handle(Seeder.Success resp) {
      LOG.debug("{}connect to:{} success", logPrefix, resp.target);
  ...
  Handler handleTransferReport = new Handler<TransferTrackingReport>() {
    ...
    public void handle(TransferTrackingReport event) {
      LOG.debug("{}transfer report", logPrefix);
  ...
  private void tryAdvance() {
    ...
    if (fileId != null) {
      ...
      if (fileWriter.isComplete()) {
        ...
        getFilesState.killInstances(fileId, connMngr.closeFileConnection(fileId));
        fileWriteClose(fileId);
        return;
      }
      int batchSize = 2;
    ...
    LOG.debug("{}advance - waiting", logPrefix);
  }
...
//**************************************************************************
  ...
  Handler handleCompletedBlocks = new Handler<CompletedBlocks>() {
    ...
    public void handle(CompletedBlocks event) {
      LOG.info("{}conn:{} completed blocks:{} hashes:{}", new Object[]{logPrefix, event.connId, event.blocks.keySet(),
        event.hashes.keySet()});
  ...
  public class GetDefinitionState {
    ...
    public void startInstance(KAddress peer) {
      ...
      leecherComp = create(DwnlConnComp.class, new DwnlConnComp.Init(connId, selfAdr, peer, MyTorrent.defaultDefBlock,
        false));
  ...
  public class ServeDefinitionState {
    ...
    public void startInstance(KAddress peer) {
      ...
      Component transferComp = create(UpldConnComp.class, new UpldConnComp.Init(connId, selfAdr,
        td.manifest.defaultBlock, false));
  ...
  public class GetFilesState {
    ...
    public void newPeerConnection(TorrentConnMngr.NewPeerConnection peerConnect) {
      LOG.info("{}get files - file:{} waiting connect to peer:{}", new Object[]{logPrefix, peerConnect.connId.fileId,
        peerConnect.peer});
    ...
    public void fileConnected(ConnId connId, KAddress peer) {
      ...
      Component leecherComp = create(DwnlConnComp.class, new DwnlConnComp.Init(connId, selfAdr, peer,
        MyTorrent.defaultDataBlock, true));
      connect(leecherComp.getNegative(Timer.class), timerPort, Channel.TWO_WAY);
      connect(leecherComp.getPositive(TransferTrackingPort.class), transferTrackingComp.getNegative(
        TransferTrackingPort.class), Channel.TWO_WAY);
      transferNetworkChannel.addChannel(connId, leecherComp.getNegative(Network.class));
      dwnlConnChannel.addChannel(connId, leecherComp.getPositive(DwnlConnPort.class));
      trigger(Start.event, leecherComp.control());
