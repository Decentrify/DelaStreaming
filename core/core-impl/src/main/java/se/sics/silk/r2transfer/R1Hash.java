...
public class R1Hash {
  ...
  public static final String NAME = "dela-r1-hash-fsm";
  ...
  public static class FSM {
    ...
    private static FSMBuilder.SemanticDefinition semanticDef() throws FSMException {
      return FSMBuilder.semanticDef()
        ...
        .basicEvent(R2TorrentTransferEvents.HashReq.class)
        .subscribeOnStart(Handlers.hash)
        .basicEvent(R2TorrentTransferEvents.HashStop.class)
  ...
  public static class Handlers {
    static FSMBasicEventHandler basicDefault() {
      return new FSMBasicEventHandler<ES, IS, KompicsEvent>() {
        ...
        public FSMStateName handle(FSMStateName state, ES es, IS is, KompicsEvent req) {
          if (req instanceof R1Metadata.TransferEvent) {
    ...
    static FSMBasicEventHandler hash = new FSMBasicEventHandler<ES, IS, R2TorrentTransferEvents.HashReq>() {
      @Override
      public FSMStateName handle(FSMStateName state, ES es, IS is, R2TorrentTransferEvents.HashReq req) {
        sendTransfer(es, req.success());
        return States.HASH;
      }
    };
    
    static FSMBasicEventHandler stop1 = new FSMBasicEventHandler<ES, IS, R2TorrentTransferEvents.HashStop>() {
      @Override
      public FSMStateName handle(FSMStateName state, ES es, IS is, R2TorrentTransferEvents.HashStop req) {
        sendTransfer(es, req.ack());
        return FSMBasicStateNames.FINAL;
      }
    };
    
    static FSMBasicEventHandler stop2 = new FSMBasicEventHandler<ES, IS, R2TorrentTransferEvents.HashStop>() {
      @Override
      public FSMStateName handle(FSMStateName state, ES es, IS is, R2TorrentTransferEvents.HashStop req) {
        sendTransfer(es, req.ack());
        return FSMBasicStateNames.FINAL;
